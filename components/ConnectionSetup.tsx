...
export const ConnectionSetup: React.FC<ConnectionSetupProps> = ({ onSkip, onSuccess }) => {
  ...
  // Simply set loading state based on hook state
  useEffect(() => {
    setInitialLoading(!initialized);
  }, [initialized]);
  ...
  const handleRefreshConnection = async () => {
    ...
    try {
      await loadConnections(true); // Force reload
    } catch (err) {
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <button
                  ...
                  className="flex items-center justify-center space-x-2 px-6 py-3 bg-emerald-600/20 hover:bg-emerald-600/30 text-emerald-400 hover:text-emerald-300 rounded-xl font-semibold transition-all duration-200 border border-emerald-500/20 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {initialLoading ? (
